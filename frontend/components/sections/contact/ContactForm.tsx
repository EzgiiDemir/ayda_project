...
export default function ContactForm({ locale }: Props) {
    ...
    // UI çıktısıyla birebir metinler (fallback'lar)
    const labels = {
        ...
        // UI’de “Gönder” yazıyor (Mesaj alanının etiketi)
    ...
    const onFinish = async (values: any) => {
        try {
            // await fetch('/api/contact', { method: 'POST', body: JSON.stringify({ ...values, locale }) });
    ...
    return (
        <Form
            form={form}
            layout="vertical"
            className="ant-form ant-form-vertical w-full max-w-2xl mx-auto flex flex-col items-center"
            onFinish={onFinish}
        ...
        >
                ...
                <Form.Item
                    ...
                    label={
                        <label htmlFor="name" className="ant-form-item-required">
                            <p className="text-[14px] leading-5 text-[#474D66]">{labels.name}</p>
                ...
                <Form.Item
                    ...
                    label={
                        <label htmlFor="email" className="ant-form-item-required">
                            <p className="text-[14px] leading-5 text-[#474D66]">{labels.email}</p>
                            <span className="text-red-500 ml-[2px]">*</span>
                        </label>
                    }
                    rules={[
            ...
            <Form.Item
                ...
                label={
                    <label htmlFor="subject" className="ant-form-item-required">
                        <p className="text-[14px] leading-5 text-[#474D66]">{labels.subject}</p>
                        <span className="text-red-500 ml-[2px]">*</span>
                    </label>
                }
            ...
            <Form.Item
                ...
                label={
                    <label htmlFor="message" className="ant-form-item-required">
                        <p className="text-[14px] leading-5 text-[#474D66]">{labels.message}</p>
                        <span className="text-red-500 ml-[2px]">*</span>
                    </label>
                }
            ...
            <Button
                htmlType="submit"
                type="primary"
                className="mx-auto bg-ayda-pink-dark px-5 md:px-8 py-2 md:py-4 rounded-full cursor-pointer lg:hover:bg-ayda-blue transition-colors duration-300 ease-in mt-4 md:mt-6 flex items-center justify-center gap-2"
            >
                <p className="text-sm md:text-base text-ayda-white capitalize font-medium">
                    {labels.submit}
                </p>
            </Button>
        </Form>
